      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Provide the main details about your game.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label htmlFor="title">Title</label>
              <Input id="title" placeholder="Your game title" {...register('title')} />
              {errors.title && <p className="text-red-500 text-sm">{errors.title.message}</p>}
            </div>
            <div>
              <label htmlFor="description">Description</label>
              <Textarea id="description" placeholder="Tell us about your game" {...register('description')} />
              {errors.description && <p className="text-red-500 text-sm">{errors.description.message}</p>}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="developer">Developer</label>
                <Input id="developer" placeholder="Your studio name" {...register('developer')} />
                {errors.developer && <p className="text-red-500 text-sm">{errors.developer.message}</p>}
              </div>
              <div>
                <label htmlFor="publisher">Publisher</label>
                <Input id="publisher" placeholder="Publishing company" {...register('publisher')} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="genre" placeholder="Action, RPG, etc." {...register('genre')} />
                {errors.genre && <p className="text-red-500 text-sm">{errors.genre.message}</p>}
              </div>
              <div>
                <label htmlFor="tags">Tags</label>
                <Input id="tags" placeholder="Indie, Multiplayer, etc." {...register('tags')} />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Upload images and videos for your game.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label htmlFor="image_url">Cover Image URL</label>
              <Input id="image_url" placeholder="https://example.com/image.png" {...register('image_url')} />
              {errors.image_url && <p className="text-red-500 text-sm">{errors.image_url.message}</p>}
            </div>
            <div className="text-center text-sm text-muted-foreground">OR</div>
            <div>
              <label htmlFor="image_file">Upload Cover Image</label>
              <Input id="image_file" type="file" {...register('image_file')} ref={fileInputRef} />
            </div>
            {(imageUrl || (imageFile && imageFile[0])) && (
              <div className="mt-4">
                <p className="text-sm font-medium mb-2">Image Preview:</p>
                <Image
                  src={imageUrl || (imageFile && URL.createObjectURL(imageFile[0]))}
                  alt="Image Preview"
                  width={200}
                  height={100}
            ...
            <div>
              ...
              <Input id="screenshots" placeholder="Comma-separated URLs" {...register('screenshots')} />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Set the price and release date for your game.</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="price">Price</label>
              <Input id="price" type="number" step="0.01" placeholder="$19.99" {...register('price')} />
              {errors.price && <p className="text-red-500 text-sm">{errors.price.message}</p>}
            </div>
            <div>
              <label htmlFor="discount_price">Discount Price</label>
              <Input id="discount_price" type="number" step="0.01" placeholder="$9.99" {...register('discount_price')} />
            ...
            <div>
              ...
              <Input id="file_url" placeholder="https://example.com/game.zip" {...register('file_url')} />
              {errors.file_url && <p className="text-red-500 text-sm">{errors.file_url.message}</p>}
            </div>
          </CardContent>
        </Card>

        <Button type="submit" size="lg" className="w-full">Publish Game</Button>
      </form>
    </div>
  );
}
