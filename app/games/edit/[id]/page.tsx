...
type UpdateGameForm = z.infer<typeof updateGameSchema>;
...
export default function EditGamePage() {
  ...
  useEffect(() => {
    if (id) {
      const fetchGame = async () => {
        setLoading(true);
        try {
          const response = await fetch(`/api/games/get-game?id=${id}`);
          if (response.ok) {
            const game = await response.json();
            // Populate the form with the fetched game data
            setValue('title', game.title);
            setValue('description', game.description);
            setValue('price', game.price);
            setValue('discount_price', game.discount_price);
            setValue('genre', game.genre.join(', '));
            setValue('tags', game.tags.join(', '));
            setValue('developer', game.developer);
            setValue('publisher', game.publisher);
            setValue('release_date', new Date(game.release_date).toISOString().split('T')[0]);
            setValue('image_url', game.image_url);
            setValue('screenshots', game.screenshots.join(', '));
            setValue('video_url', game.video_url);
  ...
  const onSubmit = async (data: UpdateGameForm) => {
    ...
    try {
      ...
      const response = await fetch(`/api/seller/games`,
      {
        method: 'PUT',
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to update game: ${errorData.message || 'Unknown error'}`);
      }
    } catch (err) {
      console.error(err);
  ...
  if (authLoading || loading || !user || !user.email_verified) {
    return <p>Loading...</p>;
  }
    ...
    <div className="container mx-auto py-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <Button type="submit" className="w-full" variant='outline'>Update Game</Button>
        </div>
      </form>
    </div>
  );
